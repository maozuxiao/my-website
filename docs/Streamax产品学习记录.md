...
> 1. **检查电源连接**：确保 MDVR 已正确连接到电源插座，且电源线无损坏，确保MDVR 的电源稳定，无电压过低或波动情况。可尝试更换其他电源插座进行测试，若问题依旧，可能是电源适配器故障，需更换电源适配器。
...
> 5. **核对设置参数**：进入 MDVR 设置界面，检查日期、时间设置是否正确，是否有定时录像、报警录像设置。查看视频编码格式、分辨率、帧率等参数是否设置正确。如果参数设置不正确，按实际情况进行调整。
...
> 1. 准备一个 U 盘，并将其连接到电脑。
> 2. 在 U 盘根目录下，找到名为 “upgrade” 的文件夹。
> 3. 将 “upgrade” 文件夹重命名为 “autoupgrade”。(如果没有找到相关文件夹，则新建一个即可。)
> 4. 把需要用于升级的文件复制并粘贴到 “autoupgrade” 文件夹内。
> 5. 确保所有升级文件已正确放置后，安全弹出 U 盘。
> 6. 将该 U 盘插入目标MDVR设备，设备会自动识别并开始升级流程。
...
> **息屏设置**：
>
> ![CP4_Settings_Vol_Bri](https://cdn.jsdelivr.net/gh/maozuxiao/Image-shack/CP4_Settings_Vol_Bri.mp4)
